...
#提取tez任务日志中的统计数据
def extract_tez_sql(tez_queue,app_id):
...
def main(file_path):
    ...
    database = 'ds'
    tablename = 'yarn_application_sql'
    columns = "application_id,dag_id,sql"
...
if __name__=='__main__':
    ...
    with connection.cursor() as cursor:
        sql = "select id from yarn_applications ya where FROM_UNIXTIME(ya.finishedTime / 1000,'%%Y%%m%%d%%H') = '%s'" % (previous_hour)
        print sql
        cursor.execute(sql)
